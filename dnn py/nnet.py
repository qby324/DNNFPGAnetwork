...
def loadnnet(fname):
    ...
    def getextraction(nucid, line):
        ...
        if m:
            ...
            pre=m.group("inl")
            if pre:
                prevnuclids = map(int, re.findall("(\d+)", m.group("inl")))#找到所有与inl匹配的数放入pre中
            elif 0<nucid:
                prevnuclids = [nucid-1]#若没有匹配则默认前一层
            else:
                prevnuclids = []
        else:
            print >> sys.stderr, "Error: Unexpected format : " + line
    ...
    for i in range(1,len(nuclei)):
        nuclei[i].setPrevNuclei(nuclei)#根据之前设置的prenuclids把相应的层链接在prenucl上
...
def expandContext(fea,
    ...
    tmpl = []
    for i in range(spl):
        tmpl.append(numpy.concatenate([numpy.zeros([spl-i, dim], dtype="float32"), fea[0:num-spl+i]], axis=1))
    tmpl.append(fea)
    for i in range(spl):
        tmpl.append(numpy.concatenate([fea[i+1:num], numpy.zeros([i+1, dim], dtype="float32")], axis=1))
...
def prepX(listX,        #trainXlist
           dataXall,    #floatX型空数组
           mnstd,       #空数组
           typeX,
           bbsize,  #big batch size 0 means all data
           lot,     #each loop add 1
           spl,     #0
           trmspl,  #0
           shflg):  #false
    ...
    if typeX in ("htkscp", "arkscp"):
        ...
        st = lot*bbsize
        ed = (lot+1)*bbsize
        idlist = scp2idlist(listX[st:ed], typeX)    #根据typeX是htkscp和arkscp不同返回不同
    else:   #不会执行
        if bbsize==0:
            bbsize = len(dataXall)
        st = lot*bbsize
