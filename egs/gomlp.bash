...
bpepoch=20     # number of epochs for fine tuning
...
for rbmi in `seq 2 3`; do
    ...
    prev=$(($rbmi - 1))
    $rbmtrain $xtrain $outsize --xType=$xType --spl=$splice --mvnf=$mvnormf --ft=$outdir/trans.${prev} --sd=1234 \
	--rt=bb --lr=0.005 --mm=0.9 --rgl=0.0001  --of=$outdir/rbm.$rbmi --wt=$outdir/trans.${rbmi} --ep=$rbmepoch
done
...
$backprop $outdir/mlp.ini $xtrain $ytrain --xType=$xType --spl=$splice --mvnf=$mvnormf --yType=$yType \
    --devX=$xvalid --devY=$yvalid --ep=$bpepoch --ot=c --lr=1.0 --mm=0.0 --rgl=0.0  --ofs=$outdir/mlp
(cd $outdir; rm -f mlp.final; ln -s mlp.$bpepoch mlp.final)


############ Export output of NN as HTK feature file ###################
mkdir -p $nnfeaout
$nnetgenhtkfea $outdir/mlp.final $xvalid $nnfeaout --spl=2 --mvnf=$mvnormf  --trmspl=False
